/**
 * Created by annayushina on 11/12/16.
 */
...
/**
 * Created by annayushina on 09/12/16.
 */
fun main(args: Array<String>) {
    val input = """gbc-frperg-pubpbyngr-znantrzrag-377[rgbnp]
...
jyfvnlupj-ibuuf-svnpzapjz-851[gmsnf]""".lines()

    for (line in input) {
        line
    }
    fun validRoom(room: String): Int {
        val last = room.split("-").takeLast(1).joinToString().split("[")
        val sum = last.take(1).joinToString().toInt()
        val key: String = last[last.lastIndex].filter { it != ']' }
        val letters = room.split("-").dropLast(1).joinToString("")
        //     val list: MutableList<Pair<Char, Int>> = mutableListOf()

        val letterGroups: Map<Char, List<Char>> = letters.groupBy { it }
        val r: List<Map.Entry<Char, List<Char>>> = letterGroups.entries
                .sortedWith(compareByDescending<Map.Entry<Char, List<Char>>> { it.value.size }.thenBy { it.key }).take(5)

        /*   for (a in 'a'..'z') {

               list.add(Pair(a, letters.count { it == a }))

           }


           list.sortByDescending { it.second }
           val list5: List<Pair<Char, Int>> = list.subList(0, 5)
        */
        val string = r.map { it.key }.joinToString("")



        return if (string == key) sum else 0
    ...
    fun rotate(ch: Char, number: Int): Char =
            'a' + (ch - 'a' + number) % 26

    fun decryption(room: String): Pair<String, Int>? {

        val sectorID = validRoom(room)

        if (sectorID <= 0) return null

        val string = room.split("-").dropLast(1).map {
            it.map { rotate(it, sectorID) }
                    .joinToString("")
        }.joinToString(" ")
        return Pair(string, sectorID)


    }


    //   val ResultSum = input.sumBy { validRoom(it) }

    val decrypted = input.mapNotNull { decryption(it) }


    decrypted.forEach(::println)
    println("_________________________")
    println(decrypted.find { it.first.contains("north") })
    println(decrypted.sumBy { it.second })

    /*  for (room in input){
          val valid = validRoom(room)

          if (valid>0) {
              println("${decryption(room, valid)} $valid")
          }

          if (room.contains("north pole objects")){
              println()
          }
      }*/
